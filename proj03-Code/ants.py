...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        goal_place = self.place
        distance = 0
        while not self.min_range <= distance and not goal_place.entrance.is_hive:
            goal_place = goal_place.entrance
            distance = distance + 1
        while goal_place.bees == [] and not goal_place.entrance.is_hive \
            and distance < self.max_range:
            goal_place = goal_place.entrance
            distance = distance + 1
        return random_bee(goal_place.bees)  # REPLACE THIS LINE
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        """
        # BEGIN Problem 5
        local_bees = self.place.bees[:]
        for bee in local_bees:
...
# BEGIN Problem 6
class WallAnt(Ant):
    """WallAnt do nothing but has a large health"""
...
# BEGIN Problem 7
class HungryAnt(Ant):
    """HungryAnt eats a random bee in it's place but need tuns to chew"""
    ...
    def eat(self, target):
        """Reduce the target's health to 0"""
...
class TankAnt(ContainerAnt):
    """TankAnt provides protection and deal damage to all bees in it's place"""
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        """
        # BEGIN Problem 12
        def double_damage(queen, ant):
            if ant != None and not ant in queen.buffedants:
                ant.damage = ant.damage * 2
                queen.buffedants.append(ant)

        super().action(gamestate)
        place = self.place.exit
        while place != None:
            ant = place.ant
            double_damage(self, ant)
            if ant != None and ant.is_container:
                double_damage(self, ant.ant_contained)
            place = place.exit
        # END Problem 12
...
class Bee(Insect):
    ...
    def __init__(self, health, place=None, slow_lefttruns=0, scary_leftturns=0, \
        once_scared=False):
        super().__init__(health, place)
        self.slow_lefttruns = slow_lefttruns
        self.scary_leftturns = scary_leftturns
        self.once_scared = once_scared
    ...
    def action(self, gamestate):
        ...
        if self.scary_leftturns > 0:
            destination = self.place.entrance
        whether_canmove = self.health > 0 and destination is not None and \
            not destination.is_hive and (self.slow_lefttruns == 0 or \
                (self.slow_lefttruns > 0 and gamestate.time % 2 == 0))
        if self.blocked():
            self.sting(self.place.ant)
        elif whether_canmove:
            self.move_to(destination)
            if self.scary_leftturns > 0:
                self.scary_leftturns -= 1
        if self.slow_lefttruns > 0:
            self.slow_lefttruns -= 1
    ...
    def slow(self, length):
        ...
        # BEGIN Problem EC
        self.slow_lefttruns = self.slow_lefttruns + length
        # END Problem EC
    ...
    def scare(self, length):
        ...
        """
        # BEGIN Problem EC
        self.scary_leftturns = self.scary_leftturns + length
        self.once_scared = True
        # END Problem EC
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        # BEGIN Problem EC
        if target and not target.once_scared:
            target.scare(2)
        # END Problem EC
...
class LaserAnt(ThrowerAnt):
    ...
    def __init__(self, health=1, total_damage=2):
        super().__init__(health)
        self.insects_shot = 0
    ...
    def insects_in_front(self):
        ...
        for bee in self.place.bees:
            insects[bee] = self.distance(bee)
        place = self.place.entrance
        while not place.is_hive:
            ant_local = place.ant
            if ant_local is not None:
                insects[ant_local] = self.distance(ant_local)
                if ant_local.is_container and ant_local.ant_contained is not None:
                    insects[ant_local.ant_contained] = self.distance(ant_local.ant_contained)
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 2
        damage = self.total_damage - distance * 0.25 - self.insects_shot * 0.0625
        return max(0, damage)
        # END Problem Optional 2
    ...
    def distance(self, insect):
        def helper(place, target):
            assert place is not None
            if place == target.place:
                return 0
            return 1 + helper(place.entrance, target)
        return helper(self.place, insect)
...
class GameState:
    ...
    def __init__(self, strategy, beehive, ant_types, create_places, dimensions, food=2, \
        ...
        self.active_bees = []
        self.has_queenant = has_queenant
