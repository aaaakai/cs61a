...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        procedure_ins = do_lambda_form(lambda_expressions, env)
        env.define(name, procedure_ins)
        return name
        # END PROBLEM 10
    else:
...
def do_and_form(expressions, env):
    ...
    if expressions.rest is nil:
        return scheme_eval(first_value, env)
...
def do_or_form(expressions, env):
    ...
    if expressions is nil:
        return False
    first_value = scheme_eval(expressions.first, env)
    if is_scheme_true(first_value) or expressions.rest is nil:
        return first_value
