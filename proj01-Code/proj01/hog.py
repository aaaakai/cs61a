...
def roll_dice(current_score, num_rolls, dice=six_sided):
    ...
    while roll_time <= num_rolls:
        roll_points = dice()
        points_sum = points_sum + roll_points
        if roll_points == 1:
            pig_out = True
        if roll_points % 2 == current_score % 2:
            feral_hogs = feral_hogs + 1
        roll_time = roll_time + 1
    if pig_out:
        points_sum = 1
    return points_sum + feral_hogs
    # END PROBLEM 1
...
def picky_piggy(score):
    ...
    def digit2num(digit):
        ...
        points = digit_num.get(mo,None)
        return points
    if score == 0:
        return 1
    else:
        return digit2num(score)
    # END PROBLEM 2
...
def swine_swap(player_score, opponent_score):
    ...
    def get_firstdigit(number):
        while number // 10 != 0:
            number = number // 10
        return number
    def get_lastdigit(number):
        return number % 10
    power = 3 ** (player_score + opponent_score)
    if get_firstdigit(power) == get_lastdigit(power):
        return True
    else:
        return False
    # END PROBLEM 4
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say0(score0, score1):
        gained_score = score0 - last_score
        highest_record = running_high
        if gained_score > running_high:
            highest_record = gained_score
            print(gained_score,'point(s)! That\'s a record gain for Player',str(who)+'!')
        return announce_highest(who, last_score = score0, running_high=highest_record)
    say1 = lambda score0, score1:say0(score1, score0)
    if who == 0:
        return say0
    else:
        return say1
