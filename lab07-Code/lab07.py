...
class Complex:
    ...
    def __add__(self, another):
        return Complex(self.real+another.real, self.imaginary+another.imaginary)

    def __mul__(self, another):
        new_real = self.real * another.real - self.imaginary * another.imaginary
        new_imaginary = self.real * another.imaginary + self.imaginary * another.real
...
def store_digits(n):
    ...
    def digits(num):
        if num // 10 == 0:
            return [num]
        return digits(num // 10) + digits(num % 10)

    def helper(digits_list):
        if len(digits_list) == 1:
            return Link(digits_list[0])
        return Link(digits_list[0], helper(digits_list[1:]))
...
def make_to_string(front, mid, back, empty_repr):
    ...
    def link_to_string(link):
        if link == Link.empty:
            return empty_repr
        return front + str(link.first) + mid + link_to_string(link.rest) + back
...
def cumulative_mul(t):
    ...
    if not t.is_leaf():
        ...
        for sub_tree in t.branches:
            t.label = t.label * sub_tree.label
