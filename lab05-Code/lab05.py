def count(t, n, x):
    ...
    if n == 0:
        return 0
    if next(t) == x:
        return 1 + count(t, n - 1, x)
    return count(t, n - 1, x)
...
def repeated(t, k):
    ...
    """
    assert k >= 1
    def helper(iterator, last_item, appear_time):
        if appear_time == 0:
            return last_item
        current_item = next(iterator)
        if current_item == last_item:
            return helper(iterator, current_item, appear_time - 1)
        return helper(iterator, current_item, k - 1)
    return helper(t, None, k)
...
def merge(a, b):
    ...
    item_of_b = next(b)
    while True:
        if item_of_a < item_of_b:
            if not item_of_a == pre_item:
                pre_item = item_of_a
                yield pre_item
            item_of_a = next(a)
        else:
            if not item_of_b == pre_item:
                pre_item = item_of_b
                yield pre_item
            item_of_b = next(b)    
